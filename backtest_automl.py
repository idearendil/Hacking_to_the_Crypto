import os
import pandas as pd
import matplotlib.pyplot as plt
from autogluon.tabular import TabularPredictor

# -----------------------------
# 기본 설정
# -----------------------------
INPUT_DIR = "G:/preprocessed_data_day_3500"
SEED_MONEY = 1000000
LABEL = "label"
MODEL_PATH = "ag_models"  # predictor 저장된 경로로 바꿔주세요

predictor = TabularPredictor.load(MODEL_PATH)

# -----------------------------
# CSV 불러오기
# -----------------------------
coin_data = {}
for file in os.listdir(INPUT_DIR):
    if file.endswith(".csv"):
        coin = file.replace(".csv", "")
        df = pd.read_csv(os.path.join(INPUT_DIR, file))
        df = df.tail(61).reset_index(drop=True)  # 마지막 60일 + 이전날 입력용 1개
        coin_data[coin] = df

# -----------------------------
# 백테스트 로직
# -----------------------------
capital = SEED_MONEY
holding_coin = None
buy_price = None
holding_days = 0

records = []  # 거래 기록 저장

for i in range(1, 61):
    today_data = {c: df.iloc[i] for c, df in coin_data.items()}
    prev_data = {c: df.iloc[i - 1] for c, df in coin_data.items()}  # 모델 입력용 (전날)

    if holding_coin is None:
        # 투자 안 한 경우 → 모델 inference
        preds = {}
        for c, row in prev_data.items():
            X = pd.DataFrame([row.drop([LABEL], errors="ignore")])
            prob = predictor.predict_proba(X)[1].iloc[0]  # class=1 확률
            preds[c] = prob

        # 가장 높은 확률 코인 선택
        best_coin = max(preds, key=preds.get)
        buy_price = today_data[best_coin]["open"]
        holding_coin = best_coin
        holding_days = 1

        records.append({
            "date": i,
            "action": "BUY",
            "coin": best_coin,
            "price": buy_price,
            "capital": capital
        })

    else:
        holding_days += 1
        row = today_data[holding_coin]
        low, high, open_price = row["low"], row["high"], row["open"]

        sell = False
        sell_price = None

        if holding_days >= 3:
            sell = True
            sell_price = open_price
        else:
            if low <= buy_price * 0.99:
                sell = True
                sell_price = buy_price * 0.99
            elif high >= buy_price * 1.02:
                sell = True
                sell_price = buy_price * 1.02

        if sell:
            capital = capital * (sell_price / buy_price)
            records.append({
                "date": i,
                "action": "SELL",
                "coin": holding_coin,
                "price": sell_price,
                "capital": capital
            })
            holding_coin = None
            buy_price = None
            holding_days = 0
            

# -----------------------------
# 결과 저장 및 그래프
# -----------------------------
result_df = pd.DataFrame(records)
print(result_df)
print(f"최종 자본: {capital:.2f} 원")

# 그래프 그리기
plt.figure(figsize=(12, 6))
plt.plot(result_df["date"], result_df["capital"], marker="o", linestyle="-", label="Capital")
plt.xticks(rotation=45)
plt.xlabel("Date")
plt.ylabel("Capital (KRW)")
plt.title("Backtest Capital Curve")
plt.legend()
plt.tight_layout()
plt.savefig("backtest_result.png", dpi=300)
plt.close()

print("그래프 저장 완료: backtest_result.png")